...
#    print('corpus: ', len(corpus), '\n')
   ...
   # print('JJR: ', chunk_dist['JJR'])
    ...
    """
    # FOR EACH POS, HOW FREQUENT ARE THE CHUNKS? DICT OF DICTS
    ...
    """
    chunk_guess = {key: '' for key in pos_cnt.keys()}
    # Adds the most probable chunk to each pos
    for pos in chunk_dist:
        most_freq = 0
        most_common = ''
        for chunk in chunk_dist[pos]:
            freq = chunk_dist[pos][chunk]
            if freq > most_freq:
                most_common = chunk
                most_freq = freq
        chunk_guess[pos] = most_common
